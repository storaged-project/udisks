...
static void
print_object (StoragedObject *object,
              guint        indent)
{
...
lookup_object_by_path (const gchar *path)
{
  ...
  s = g_strdup_printf ("/org/storaged/Storaged/%s", path);
  ret = g_dbus_object_manager_get_object (storaged_client_get_object_manager (client), s);
  g_free (s);

  if (ret == NULL)
    return NULL;
  else
    return STORAGED_OBJECT (ret);
}
      ...
      if (block != NULL)
        {
          ...
          if (g_strcmp0 (storaged_block_get_device (block), device) == 0)
            {
...
lookup_object_by_drive (const gchar *drive)
{
  ...
  full_drive_object_path = g_strdup_printf ("/org/storaged/Storaged/drives/%s", drive);

  objects = g_dbus_object_manager_get_objects (storaged_client_get_object_manager (client));
  for (l = objects; l != NULL; l = l->next)
    {
      StoragedObject *object = STORAGED_OBJECT (l->data);
      StoragedDrive *drive_iface;

      if (g_strcmp0 (g_dbus_object_get_object_path (G_DBUS_OBJECT (object)), full_drive_object_path) != 0)
        continue;

      drive_iface = storaged_object_peek_drive (object);
      if (drive_iface != NULL)
        {
          ret = g_object_ref (object);
          goto out;
          ...
          if ((is_mount && !is_mounted) || (!is_mount && is_mounted))
            {
              ...
              g_assert (g_str_has_prefix (object_path, "/org/storaged/Storaged/"));
              g_print ("%s \n", object_path + sizeof ("/org/storaged/Storaged/") - 1);
            }
        }
      g_list_foreach (objects, (GFunc) g_object_unref, NULL);
      g_list_free (objects);
  ...
  if (is_mount)
    {
      ...
      error = NULL;
      if (!storaged_filesystem_call_mount_sync (filesystem,
                                              options,
                                              &mount_path,
                                              NULL,                       /* GCancellable */
                                              &error))
        {
          if (error->domain == STORAGED_ERROR &&
              error->code == STORAGED_ERROR_NOT_AUTHORIZED_CAN_OBTAIN &&
...
static gboolean
encrypted_is_unlocked (StoragedObject *encrypted_object)
{
  GList *objects;
  GList *l;
  ...
  for (l = objects; l != NULL; l = l->next)
    {
      ...
      if (block != NULL)
        {
          if (g_strcmp0 (storaged_block_get_crypto_backing_device (block), encrypted_object_path) == 0)
            {
              ret = TRUE;
              goto out;
            }
      ...
      for (l = objects; l != NULL; l = l->next)
        {
          ...
          object = STORAGED_OBJECT (l->data);
          encrypted = storaged_object_peek_encrypted (object);
  ...
  if (is_unlock)
    {
      ...
      error = NULL;
      if (!storaged_encrypted_call_unlock_sync (encrypted,
                                              passphrase,
                                              options,
      ...
      storaged_client_settle (client);

      cleartext_object = STORAGED_OBJECT (g_dbus_object_manager_get_object (storaged_client_get_object_manager (client),
                                                                          (cleartext_object_path)));
      g_print ("Unlocked %s as %s.\n",
               storaged_block_get_device (block),
               storaged_block_get_device (storaged_object_get_block (cleartext_object)));
  ...
  else
    {
      ...
      error = NULL;
      if (!storaged_encrypted_call_lock_sync (encrypted,
                                            options,
                                            NULL,         /* GCancellable */
                                            &error))
        {
          ...
          if (complete_objects)
            {
              ...
              for (l = objects; l != NULL; l = l->next)
                {
                  object = STORAGED_OBJECT (l->data);
                  if (storaged_object_peek_loop (object) != NULL)
                    {
                      object_path = g_dbus_object_get_object_path (G_DBUS_OBJECT (object));
                      g_assert (g_str_has_prefix (object_path, "/org/storaged/Storaged/"));
                      g_print ("%s \n", object_path + sizeof ("/org/storaged/Storaged/") - 1);
  ...
  if (is_setup)
    {
      ...
      StoragedObject *resulting_object;
      GUnixFDList *fd_list;
      gint fd;
      gboolean rc;
    ...
    setup_try_again:
      error = NULL;
      rc = storaged_manager_call_loop_setup_sync (storaged_client_get_manager (client),
      ...
      resulting_object = STORAGED_OBJECT (g_dbus_object_manager_get_object (storaged_client_get_object_manager (client),
                                                                          (resulting_object_path)));
      g_print ("Mapped file %s as %s.\n",
               opt_loop_file,
               storaged_block_get_device (storaged_object_get_block (resulting_object)));
    ...
    delete_try_again:
      error = NULL;
      if (!storaged_loop_call_delete_sync (storaged_object_peek_loop (object),
                                           options,
                                           NULL,                       /* GCancellable */
                                           &error))
        {
          ...
          g_printerr ("Error deleting loop device %s: %s\n",
                      storaged_block_get_device (storaged_object_peek_block (object)),
                      error->message);
          g_error_free (error);
          goto out;
        }
                               ...
                               const gchar *completion_prev)
{
      ...
      if (complete_objects)
        {
          ...
          for (l = objects; l != NULL; l = l->next)
            {
              object = STORAGED_OBJECT (l->data);
              ata = storaged_object_peek_drive_ata (object);
              if (ata != NULL)
                {
                  object_path = g_dbus_object_get_object_path (G_DBUS_OBJECT (object));
                  g_assert (g_str_has_prefix (object_path, "/org/storaged/Storaged/"));
              ...
              if (ata != NULL)
                {
                  const gchar * const *symlinks;
                  StoragedBlock *block;
                  block = storaged_client_get_block_for_drive (client, storaged_object_peek_drive (object), TRUE);
                  g_print ("%s \n", storaged_block_get_device (block));
                  symlinks = storaged_block_get_symlinks (block);
                  for (n = 0; symlinks != NULL && symlinks[n] != NULL; n++)
                    g_print ("%s \n", symlinks[n]);
  ...
  if (opt_smart_simulate_object_path != NULL)
  ...
  else if (opt_smart_simulate_device != NULL)
    {
      ...
      drive = storaged_client_get_drive_for_block (client, storaged_object_peek_block (block_object));
      object = (StoragedObject *) g_dbus_interface_dup_object (G_DBUS_INTERFACE (drive));
  ...
  if (storaged_object_peek_drive_ata (object) == NULL)
    {
 ...
 try_again:
  error = NULL;
  if (!storaged_drive_ata_call_smart_update_sync (storaged_object_peek_drive_ata (object),
                                                  options,
                                                  NULL,                       /* GCancellable */
                                                  &error))
    {
                          ...
                          const gchar *completion_prev)
{
 ...
 try_again:
  error = NULL;
  if (!storaged_drive_call_power_off_sync (storaged_object_peek_drive (object),
                                           options,
                                           NULL,                       /* GCancellable */
                                           &error))
    {
  ...
  if (complete_drives)
    {
      ...
      for (l = objects; l != NULL; l = l->next)
        {
          object = STORAGED_OBJECT (l->data);
          drive = storaged_object_peek_drive (object);
          if (drive != NULL)
            {
              const gchar *base;
...
obj_proxy_drive_sortkey_cmp (GDBusObject *a,
                             GDBusObject *b)
{
  StoragedDrive *da = storaged_object_peek_drive (STORAGED_OBJECT (a));
  StoragedDrive *db = storaged_object_peek_drive (STORAGED_OBJECT (b));
  if (da != NULL && db != NULL)
    return g_strcmp0 (storaged_drive_get_sort_key (da), storaged_drive_get_sort_key (db));
  else
...
monitor_has_name_owner (void)
{
  gchar *name_owner;
  gboolean ret;
  name_owner = g_dbus_object_manager_client_get_name_owner (G_DBUS_OBJECT_MANAGER_CLIENT (storaged_client_get_object_manager (client)));
  ret = (name_owner != NULL);
  g_free (name_owner);
  return ret;
}
...
monitor_print_name_owner (void)
{
  ...
  if (name_owner != NULL)
    g_print ("The storaged-daemon is running (name-owner %s).\n", name_owner);
  else
    g_print ("The storaged-daemon is not running.\n");
  g_free (name_owner);
}
                         ...
                         gpointer             user_data)
{
  ...
  g_print ("%s%sAdded %s%s\n",
           ...
           _color_get (_COLOR_RESET));
  print_object (STORAGED_OBJECT (object), 2);
 out:
  ;
}
                        ...
                        const gchar *completion_prev)
{
  ...
  g_print ("Monitoring the storaged daemon. Press Ctrl+C to exit.\n");

  manager = storaged_client_get_object_manager (client);
  ...
  for (l = objects; l != NULL; l = l->next)
    {
      ...
      for (j = blocks; j != NULL; j = j->next)
        {
          StoragedBlock *block_iface = STORAGED_BLOCK (j->data);
          GDBusObject *block_object;
          StoragedPartition *partition;
          block_object = g_dbus_interface_get_object (G_DBUS_INTERFACE (block_iface));
          partition = block_object == NULL ? NULL : storaged_object_peek_partition (STORAGED_OBJECT (block_object));
          if (partition == NULL)
            {
              const gchar *device_file;
              if (str->len > 0)
                g_string_append (str, " ");
              device_file = storaged_block_get_device (block_iface);
      ...
      revision = storaged_drive_get_revision (drive);
      serial = storaged_drive_get_serial (drive);
