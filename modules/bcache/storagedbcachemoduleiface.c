...
{
  return storaged_bcache_state_new (daemon);
}
...
storaged_module_teardown (StoragedDaemon *daemon)
{
  ...
  StoragedBcacheState *state_pointer = (StoragedBcacheState *) \
                                        storaged_module_manager_get_module_state_pointer (manager,
                                                                                          BCACHE_MODULE_NAME);

  storaged_bcache_state_free (state_pointer);
}
...
static void
bcache_block_connect (StoragedObject *object)
{
}
...
storaged_module_get_block_object_iface_setup_entries (void)
{
  ...
  iface[0]->skeleton_type = STORAGED_TYPE_LINUX_BLOCK_BCACHE;
...
new_manager_bcache_manager_iface (StoragedDaemon *daemon)
{
  StoragedLinuxManagerBcache *manager;

  manager = storaged_linux_manager_bcache_new (daemon);
