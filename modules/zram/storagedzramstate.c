...
struct _StoragedZRAMState
{
  StoragedDaemon *daemon;
};

/**
 * storaged_zram_state_new:
 * @daemon: A #StoragedDaemon instance.
 *
 * Initializes the #StoragedZRAMState structure that holds the global state
 * within ZRAM plugin.
 *
 * Returns: (transfer full): A #StoragedZRAMState that must be freed with
 * storaged_zram_state_free().
 */
