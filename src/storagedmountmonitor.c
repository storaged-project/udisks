...
/**
 * SECTION:storagedmountmonitor
 * @title: StoragedMountMonitor
 ...
 *
 * The #StoragedMountMonitor structure contains only private data and
...
typedef struct _StoragedMountMonitorClass StoragedMountMonitorClass;

struct _StoragedMountMonitorClass
{
  GObjectClass parent_class;

  void (*mount_added)   (StoragedMountMonitor  *monitor,
                         StoragedMount         *mount);
  void (*mount_removed) (StoragedMountMonitor  *monitor,
                         StoragedMount         *mount);
};
...
G_DEFINE_TYPE (StoragedMountMonitor, storaged_mount_monitor, G_TYPE_OBJECT)

static void storaged_mount_monitor_ensure (StoragedMountMonitor *monitor);
static void storaged_mount_monitor_invalidate (StoragedMountMonitor *monitor);
static void storaged_mount_monitor_constructed (GObject *object);

static void
storaged_mount_monitor_finalize (GObject *object)
{
  StoragedMountMonitor *monitor = STORAGED_MOUNT_MONITOR (object);
  ...
  if (G_OBJECT_CLASS (storaged_mount_monitor_parent_class)->finalize != NULL)
    G_OBJECT_CLASS (storaged_mount_monitor_parent_class)->finalize (object);
}

static void
storaged_mount_monitor_init (StoragedMountMonitor *monitor)
{
  monitor->mounts = NULL;
}

static void
storaged_mount_monitor_class_init (StoragedMountMonitorClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;

  gobject_class->finalize    = storaged_mount_monitor_finalize;
  gobject_class->constructed = storaged_mount_monitor_constructed;

  /**
   * StoragedMountMonitor::mount-added
   * @monitor: A #StoragedMountMonitor.
   * @mount: The #StoragedMount that was added.
   *
  ...
  signals[MOUNT_ADDED_SIGNAL] = g_signal_new ("mount-added",
                                              ...
                                              G_STRUCT_OFFSET (StoragedMountMonitorClass, mount_added),
                                              NULL,
                                              NULL,
  ...
  /**
   * StoragedMountMonitor::mount-removed
   * @monitor: A #StoragedMountMonitor.
   * @mount: The #StoragedMount that was removed.
   *
  ...
  signals[MOUNT_REMOVED_SIGNAL] = g_signal_new ("mount-removed",
                                                G_OBJECT_CLASS_TYPE (klass),
                                                G_SIGNAL_RUN_LAST | G_SIGNAL_DETAILED,
                                                G_STRUCT_OFFSET (StoragedMountMonitorClass, mount_removed),
                                                NULL,
                                                NULL,
                                                g_cclosure_marshal_VOID__OBJECT,
                                                G_TYPE_NONE,
...
static void
reload_mounts (StoragedMountMonitor *monitor)
{
  GList *old_mounts;
  GList *cur_mounts;
  GList *added;
  ...
  storaged_mount_monitor_ensure (monitor);

  old_mounts = g_list_copy (monitor->mounts);
  g_list_foreach (old_mounts, (GFunc) g_object_ref, NULL);

  storaged_mount_monitor_invalidate (monitor);
  storaged_mount_monitor_ensure (monitor);

  cur_mounts = g_list_copy (monitor->mounts);

  old_mounts = g_list_sort (old_mounts, (GCompareFunc) storaged_mount_compare);
  cur_mounts = g_list_sort (cur_mounts, (GCompareFunc) storaged_mount_compare);
  diff_sorted_lists (old_mounts, cur_mounts, (GCompareFunc) storaged_mount_compare, &added, &removed);
...
mounts_changed_event (GIOChannel *channel,
                      ...
                      gpointer user_data)
{
  StoragedMountMonitor *monitor = STORAGED_MOUNT_MONITOR (user_data);
  if (cond & ~G_IO_ERR)
    goto out;
...
static void
storaged_mount_monitor_constructed (GObject *object)
{
  StoragedMountMonitor *monitor = STORAGED_MOUNT_MONITOR (object);
  GError *error;
  ...
  if (monitor->swaps_channel != NULL)
  ...
  else
    {
      if (!(error->domain == G_FILE_ERROR && error->code == G_FILE_ERROR_NOENT))
        {
          storaged_warning ("Error opening /proc/swaps file: %s (%s, %d)",
                          error->message, g_quark_to_string (error->domain), error->code);
        }
      g_error_free (error);
    }

  if (G_OBJECT_CLASS (storaged_mount_monitor_parent_class)->constructed != NULL)
    (*G_OBJECT_CLASS (storaged_mount_monitor_parent_class)->constructed) (object);
}
 ...
 *
 * Returns: A #StoragedMountMonitor. Free with g_object_unref().
 */
StoragedMountMonitor *
storaged_mount_monitor_new (void)
{
  return STORAGED_MOUNT_MONITOR (g_object_new (STORAGED_TYPE_MOUNT_MONITOR, NULL));
}

static void
storaged_mount_monitor_invalidate (StoragedMountMonitor *monitor)
{
...
static gboolean
have_mount (StoragedMountMonitor *monitor,
            dev_t                 dev,
            const gchar          *mount_point)
{
  ...
  for (l = monitor->mounts; l != NULL; l = l->next)
    {
      StoragedMount *mount = STORAGED_MOUNT (l->data);
      if (storaged_mount_get_dev (mount) == dev &&
...
static gboolean
storaged_mount_monitor_get_mountinfo (StoragedMountMonitor  *monitor,
                                      GError               **error)
{
  gboolean ret;
  gchar *contents;
  ...
  for (n = 0; lines[n] != NULL; n++)
    {
      ...
      if (sscanf (lines[n],
        ...
        {
          storaged_warning ("Error parsing line '%s'", lines[n]);
          continue;
        }
          ...
          if (sep != NULL)
            {
              ...
              if (sscanf (sep + 3, PATH_MAX_FMT " " PATH_MAX_FMT, fstype, mount_source) != 2)
                {
                  storaged_warning ("Error parsing things past - for '%s'", lines[n]);
                  continue;
                }
              ...
              if (stat (mount_source, &statbuf) != 0)
                {
                  storaged_warning ("Error statting %s: %m", mount_source);
                  continue;
                }

              if (!S_ISBLK (statbuf.st_mode))
                {
                  storaged_warning ("%s is not a block device", mount_source);
                  continue;
                }
      ...
      if (!have_mount (monitor, dev, mount_point))
        {
          StoragedMount *mount;
          mount = _storaged_mount_new (dev, mount_point, STORAGED_MOUNT_TYPE_FILESYSTEM);
...
static gboolean
storaged_mount_monitor_get_swaps (StoragedMountMonitor  *monitor,
                                  GError               **error)
{
  gboolean ret;
  gchar *contents;
  ...
  for (n = 0; lines[n] != NULL; n++)
    {
      ...
      if (stat (filename, &statbuf) != 0)
        {
          storaged_warning ("Error statting %s: %m", filename);
          continue;
        }
      ...
      if (!have_mount (monitor, dev, NULL))
        {
          StoragedMount *mount;
          mount = _storaged_mount_new (dev, NULL, STORAGED_MOUNT_TYPE_SWAP);
          monitor->mounts = g_list_prepend (monitor->mounts, mount);
        }
    }
...
static void
storaged_mount_monitor_ensure (StoragedMountMonitor *monitor)
{
  GError *error;
  ...
  error = NULL;
  if (!storaged_mount_monitor_get_mountinfo (monitor, &error))
    {
      storaged_warning ("Error getting mounts: %s (%s, %d)",
                        error->message, g_quark_to_string (error->domain), error->code);
      g_error_free (error);
    }

  error = NULL;
  if (!storaged_mount_monitor_get_swaps (monitor, &error))
    {
      storaged_warning ("Error getting swaps: %s (%s, %d)",
                        error->message, g_quark_to_string (error->domain), error->code);
      g_error_free (error);
    }
 ...
 *
 * Returns: A #GList of #StoragedMount objects. The returned list must
 * be freed with g_list_free() after each element has been freed with
 * g_object_unref().
 */
GList *
storaged_mount_monitor_get_mounts_for_dev (StoragedMountMonitor *monitor,
                                           dev_t                 dev)
{
  GList *ret;
  GList *l;
  ...
  ret = g_list_sort (ret, (GCompareFunc) storaged_mount_compare);
 ...
 */
gboolean
storaged_mount_monitor_is_dev_in_use (StoragedMountMonitor  *monitor,
                                      dev_t                  dev,
  ...
  for (l = monitor->mounts; l != NULL; l = l->next)
    {
      ...
      if (storaged_mount_get_dev (mount) == dev)
        {
          if (out_type != NULL)
            *out_type = storaged_mount_get_mount_type (mount);
          ret = TRUE;
          goto out;
        }
    }
