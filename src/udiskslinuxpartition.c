  ...
  if (g_strcmp0 (udisks_partition_table_get_type_ (partition_table), "gpt") == 0)
    {
      ...
      if (!bd_part_set_part_type (device_name, partition_name, type, error))
        {
          g_prefix_error (error, "Error setting type for partition %s: ", partition_name);
          goto out;
        }
    }
  else if (g_strcmp0 (udisks_partition_table_get_type_ (partition_table), "dos") == 0)
    {
      ...
      if (!bd_part_set_part_id (device_name, partition_name, type, error))
        {
          g_prefix_error (error, "Error setting type for partition %s: ", partition_name);
          goto out;
        }
