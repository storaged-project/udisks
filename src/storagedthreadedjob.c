...
/**
 * SECTION:storagedthreadedjob
 * @title: StoragedThreadedJob
...
typedef struct _StoragedThreadedJobClass   StoragedThreadedJobClass;

/**
 * StoragedThreadedJob:
 *
 * The #StoragedThreadedJob structure contains only private data and should
...
struct _StoragedThreadedJobClass
{
  StoragedBaseJobClass parent_class;

  gboolean (*threaded_job_completed) (StoragedThreadedJob  *job,
                                      gboolean              result,
                                      GError               *error);
};
...
static gboolean storaged_threaded_job_threaded_job_completed_default (StoragedThreadedJob  *job,
                                                                      gboolean              result,
                                                                      GError               *error);

G_DEFINE_TYPE_WITH_CODE (StoragedThreadedJob, storaged_threaded_job, STORAGED_TYPE_BASE_JOB,
                         G_IMPLEMENT_INTERFACE (STORAGED_TYPE_JOB, job_iface_init));

static void
storaged_threaded_job_finalize (GObject *object)
{
  ...
  if (G_OBJECT_CLASS (storaged_threaded_job_parent_class)->finalize != NULL)
    G_OBJECT_CLASS (storaged_threaded_job_parent_class)->finalize (object);
}

static void
storaged_threaded_job_get_property (GObject    *object,
                                    guint       prop_id,
                                    GValue     *value,
                                    GParamSpec *pspec)
{
...
static void
storaged_threaded_job_set_property (GObject      *object,
                                    guint         prop_id,
                                    const GValue *value,
                                    GParamSpec   *pspec)
{
...
static void
storaged_threaded_job_init (StoragedThreadedJob *job)
{
}

static void
storaged_threaded_job_class_init (StoragedThreadedJobClass *klass)
{
  GObjectClass *gobject_class;

  klass->threaded_job_completed = storaged_threaded_job_threaded_job_completed_default;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize     = storaged_threaded_job_finalize;
  gobject_class->constructed  = storaged_threaded_job_constructed;
  gobject_class->set_property = storaged_threaded_job_set_property;
  gobject_class->get_property = storaged_threaded_job_get_property;

  /**
   * StoragedThreadedJob:job-func:
   *
  ...
  /**
   * StoragedThreadedJob:user-data-free-func:
   *
  ...
  /**
   * StoragedThreadedJob::threaded-job-completed:
   * @job: The #StoragedThreadedJob emitting the signal.
   * @result: The #gboolean returned by the #StoragedThreadedJobFunc.
   * @error: The #GError set by the #StoragedThreadedJobFunc.
   *
  ...
  signals[THREADED_JOB_COMPLETED_SIGNAL] =
    g_signal_new ("threaded-job-completed",
                  STORAGED_TYPE_THREADED_JOB,
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (StoragedThreadedJobClass, threaded_job_completed),
                  g_signal_accumulator_true_handled,
                  NULL,
                  storaged_daemon_marshal_BOOLEAN__BOOLEAN_BOXED,
                  G_TYPE_BOOLEAN,
                  2,
                  G_TYPE_BOOLEAN,
                  G_TYPE_ERROR);
 ...
 *
 * Returns: A new #StoragedThreadedJob. Free with g_object_unref().
 */
StoragedThreadedJob *
                           ...
                           GCancellable            *cancellable)
{
  ...
  return STORAGED_THREADED_JOB (g_object_new (STORAGED_TYPE_THREADED_JOB,
                                            "job-func", job_func,
                                            "user-data", user_data,
...
{
  g_return_val_if_fail (STORAGED_IS_THREADED_JOB (job), NULL);
...
static gboolean
storaged_threaded_job_threaded_job_completed_default (StoragedThreadedJob  *job,
                                                      gboolean              result,
                                                      GError               *error)
{
  if (result)
