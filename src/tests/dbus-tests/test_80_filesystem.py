...
class UdisksFSTestCase(udiskstestcase.UdisksTestCase):
    ...
    def test_protective_part_overwrite_mounted(self):
        ...
        msg = "Error synchronizing after initial wipe: Timed out waiting for object"
        # NB: this was supposed to be handled differently in the daemon however
        # it's not currently easily possible to detect mounted/busy partition
        # somewhere in the block device nested structure
        with six.assertRaisesRegex(self, dbus.exceptions.DBusException, msg):
            disk.Format(self._fs_name, self.no_options, dbus_interface=self.iface_prefix + '.Block')
