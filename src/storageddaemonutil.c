 ...
 */
gchar *
storaged_decode_udev_string (const gchar *str)
{
  GString *s;
  gchar *ret;
      ...
      if (str[n] == '\\')
        {
          ...
          if (str[n + 1] != 'x' || str[n + 2] == '\0' || str[n + 3] == '\0')
            {
              storaged_warning ("**** NOTE: malformed encoded string `%s'", str);
              break;
            }
  ...
  if (!g_utf8_validate (s->str, -1, &end_valid))
    {
      storaged_warning ("The string `%s' is not valid UTF-8. Invalid characters begins at `%s'", s->str, end_valid);
...
/**
 * storaged_safe_append_to_object_path:
 ...
 */
void
storaged_safe_append_to_object_path (GString      *str,
                                     const gchar  *s)
{
  guint n;
  for (n = 0; s[n] != '\0'; n++)
...
/**
 * storaged_daemon_util_block_get_size:
 ...
 */
guint64
storaged_daemon_util_block_get_size (GUdevDevice *device,
...
/**
 * storaged_daemon_util_resolve_link:
 * @path: A path
 ...
 */
gchar *
storaged_daemon_util_resolve_link (const gchar *path,
                                   const gchar *name)
{
  gchar *full_path;
...
/**
 * storaged_daemon_util_resolve_links:
 * @path: A path
 ...
 */
gchar **
storaged_daemon_util_resolve_links (const gchar *path,
                                    const gchar *dir_name)
{
  ...
  while ((name = g_dir_read_name (dir)) != NULL)
    {
      gchar *resolved;
      resolved = storaged_daemon_util_resolve_link (s, name);
      if (resolved != NULL)
...
/**
 * storaged_daemon_util_setup_by_user:
 ...
 */
gboolean
storaged_daemon_util_setup_by_user (StoragedDaemon *daemon,
                                    ...
                                    uid_t           user)
{
  ...
  /* loop devices */
  if (storaged_state_has_loop (state, storaged_block_get_device (block), &setup_by_user))
    {
  ...
  if (partition != NULL)
    {
      ...
      partition_object = storaged_daemon_find_object (daemon, storaged_partition_get_table (partition));
      if (partition_object != NULL)
        {
          if (storaged_daemon_util_setup_by_user (daemon, partition_object, user))
            {
              ret = TRUE;
  ...
  if (crypto_object != NULL)
    {
      ...
      if (storaged_state_find_unlocked_luks (state,
                                             storaged_block_get_device_number (crypto_block),
                                             &setup_by_user))
        {
          if (setup_by_user == user)
            {
  ...
  /* MDRaid devices */
  if (g_strcmp0 (storaged_block_get_mdraid (block), "/") != 0)
    {
      uid_t started_by_user;
      if (storaged_state_has_mdraid (state, storaged_block_get_device_number (block), &started_by_user))
        {
...
static gboolean
check_authorization_no_polkit (StoragedDaemon          *daemon,
                               StoragedObject          *object,
...
/**
 * storaged_daemon_util_check_authorization_sync:
 * @daemon: A #StoragedDaemon.
 ...
 *
 * See <xref linkend="storaged-polkit-details"/> for the variables that
 * can be used in @message but note that not all variables can be used
 * in all checks. For example, any check involving a #StoragedDrive or a
 * #StoragedBlock object can safely include the fragment
 ...
 * <link linkend="gdbus-interface-org-storaged-Storaged-Manager.top_of_page">Manager</link>
 * object.
 *
 * Returns: %TRUE if caller is authorized, %FALSE if not.
 */
gboolean
storaged_daemon_util_check_authorization_sync (StoragedDaemon          *daemon,
                                               StoragedObject          *object,
                                               const gchar             *action_id,
                                               GVariant                *options,
                                               const gchar             *message,
                                               GDBusMethodInvocation   *invocation)
{
  ...
  polkit_details_insert (details, "polkit.gettext_domain", "storaged");
      ...
      if (block != NULL)
        {
          ...
          if (drive_object != NULL)
            drive = storaged_object_get_drive (drive_object);
        }
  ...
  if (block != NULL)
    details_device = storaged_block_get_preferred_device (block);
  ...
  if (drive != NULL)
    {
      ...
      if (block != NULL)
        {
          details_drive = g_strdup_printf ("%s (%s)", s, storaged_block_get_preferred_device (block));
        }
      else
        {
          details_drive = s;
      ...
      _safe_polkit_details_insert (details, "drive.wwn", storaged_drive_get_wwn (drive));
      _safe_polkit_details_insert (details, "drive.serial", storaged_drive_get_serial (drive));
      _safe_polkit_details_insert (details, "drive.vendor", storaged_drive_get_vendor (drive));
      _safe_polkit_details_insert (details, "drive.model", storaged_drive_get_model (drive));
      _safe_polkit_details_insert (details, "drive.revision", storaged_drive_get_revision (drive));
      if (storaged_drive_get_removable (drive))
        {
          ...
          _safe_polkit_details_insert (details, "drive.removable.bus", storaged_drive_get_connection_bus (drive));
  ...
  if (block != NULL)
    {
      _safe_polkit_details_insert (details, "id.type",    storaged_block_get_id_type (block));
      _safe_polkit_details_insert (details, "id.usage",   storaged_block_get_id_usage (block));
      _safe_polkit_details_insert (details, "id.version", storaged_block_get_id_version (block));
      _safe_polkit_details_insert (details, "id.label",   storaged_block_get_id_label (block));
      _safe_polkit_details_insert (details, "id.uuid",    storaged_block_get_id_uuid (block));
    }

  if (partition != NULL)
    {
      _safe_polkit_details_insert_int    (details, "partition.number", storaged_partition_get_number (partition));
      _safe_polkit_details_insert        (details, "partition.type",   storaged_partition_get_type_ (partition));
      _safe_polkit_details_insert_uint64 (details, "partition.flags",  storaged_partition_get_flags (partition));
      _safe_polkit_details_insert        (details, "partition.name",   storaged_partition_get_name (partition));
      _safe_polkit_details_insert        (details, "partition.uuid",   storaged_partition_get_uuid (partition));
    }

  /* Fall back to Block:preferred-device */
  if (details_drive == NULL && block != NULL)
    details_drive = storaged_block_dup_preferred_device (block);
...
/**
 * storaged_daemon_util_get_caller_uid_sync:
 * @daemon: A #StoragedDaemon.
 ...
 */
gboolean
storaged_daemon_util_get_caller_uid_sync (StoragedDaemon            *daemon,
                                          GDBusMethodInvocation     *invocation,
...
/**
 * storaged_daemon_util_get_caller_pid_sync:
 * @daemon: A #StoragedDaemon.
 * @invocation: A #GDBusMethodInvocation.
 * @cancellable: (allow-none): A #GCancellable or %NULL.
 ...
 */
gboolean
storaged_daemon_util_get_caller_pid_sync (StoragedDaemon            *daemon,
                                          GDBusMethodInvocation     *invocation,
                                          GCancellable              *cancellable,
                                          pid_t                     *out_pid,
                                          GError                   **error)
...
/**
 * storaged_daemon_util_dup_object:
 ...
 *
 * Gets the enclosing #StoragedObject for @interface, if any.
 *
 * Returns: (transfer full) (type StoragedObject): Either %NULL or a
 * #StoragedObject<!-- -->-derived instance that must be released with
 * g_object_unref().
 */
gpointer
storaged_daemon_util_dup_object (gpointer   interface_,
                                 GError   **error)
{
  gpointer ret;
...
/**
 * storaged_daemon_util_escape_and_quote:
 * @str: The string to escape.
 *
 * Like storaged_daemon_util_escape() but also wraps the result in
 * double-quotes.
 *
 * Returns: The double-quoted and escaped string. Free with g_free().
 */
gchar *
storaged_daemon_util_escape_and_quote (const gchar *str)
{
  GString *s;
 ...
 */
gchar *
storaged_daemon_util_escape (const gchar *str)
{
  GString *s;
                                   ...
                                   pid_t                    process)
{
  ...
  if (STORAGED_IS_LINUX_BLOCK_OBJECT (object))
    {
      StoragedLinuxBlockObject *linux_block_object;
      StoragedBlock *block;
      linux_block_object = STORAGED_LINUX_BLOCK_OBJECT (object);
      block = storaged_object_get_block (STORAGED_OBJECT (linux_block_object));
      if (block != NULL)
        {
          drive_object = storaged_daemon_find_object (daemon, storaged_block_get_drive (block));
  ...
  else if (STORAGED_IS_LINUX_DRIVE_OBJECT (object))
    {
  ...
  drive = storaged_object_get_drive (STORAGED_OBJECT (drive_object));
  if (drive == NULL)
    goto out;
 ...
 */
gchar *
storaged_daemon_util_hexdump (gconstpointer data, gsize len)
{
...
/**
 * storaged_daemon_util_hexdump_debug:
 * @data: Pointer to data.
 * @len: Length of data.
 *
 * Utility function to dumps the hexadecimal representation of @len
 * bytes of @data generated with storaged_daemon_util_hexdump() using
 * storaged_debug().
 */
void
storaged_daemon_util_hexdump_debug (gconstpointer data, gsize len)
{
  gchar *s = storaged_daemon_util_hexdump (data, len);
  storaged_debug ("Hexdump of %" G_GSIZE_FORMAT " bytes:\n%s", len, s);
  g_free (s);
}
...
/**
 * storaged_daemon_util_file_set_contents:
 ...
 */
gboolean
storaged_daemon_util_file_set_contents (const gchar  *filename,
...
/**
 * StoragedInhibitCookie:
 *
 * Opaque data structure used in storaged_daemon_util_inhibit_system_sync() and
 * storaged_daemon_util_uninhibit_system_sync().
 */
...
/**
 * storaged_daemon_util_inhibit_system_sync:
 ...
 *
 * Returns: A cookie that can be used with storaged_daemon_util_uninhibit_system_sync().
 */
StoragedInhibitCookie *
storaged_daemon_util_inhibit_system_sync (const gchar  *reason)
{
#ifdef HAVE_LIBSYSTEMD_LOGIN
  StoragedInhibitCookie *ret = NULL;
  ...
  if (connection == NULL)
    {
      storaged_error ("Error getting system bus: %s (%s, %d)",
                    error->message, g_quark_to_string (error->domain), error->code);
      g_clear_error (&error);
      goto out;
    }
  ...
  if (value == NULL)
    {
      storaged_error ("Error inhibiting: %s (%s, %d)",
                    error->message, g_quark_to_string (error->domain), error->code);
      g_clear_error (&error);
      goto out;
    }
  ...
  ret = g_new0 (StoragedInhibitCookie, 1);
  ret->magic = 0xdeadbeef;
  ret->fd = g_unix_fd_list_get (fd_list, index, &error);
  if (ret->fd == -1)
    {
      storaged_error ("Error getting fd: %s (%s, %d)",
                    error->message, g_quark_to_string (error->domain), error->code);
      g_clear_error (&error);
      g_free (ret);
      ret = NULL;
 ...
 out:
  ...
  g_return_val_if_fail (reason != NULL, NULL);
  return (StoragedInhibitCookie* ) &storaged_daemon_util_inhibit_system_sync;
#endif
}

/**
 * storaged_daemon_util_uninhibit_system_sync:
 * @cookie: %NULL or a cookie obtained from storaged_daemon_util_inhibit_system_sync().
 *
 * Does nothing if @cookie is %NULL, otherwise uninhibits.
 */
void
storaged_daemon_util_uninhibit_system_sync (StoragedInhibitCookie *cookie)
{
#ifdef HAVE_LIBSYSTEMD_LOGIN
  if (cookie != NULL)
    {
      ...
      if (close (cookie->fd) != 0)
        {
          storaged_error ("Error closing inhbit-fd: %m");
        }
      g_free (cookie);
    }
#else
  /* non-systemd: check dummy pointer */
  g_warn_if_fail (cookie == (StoragedInhibitCookie* ) &storaged_daemon_util_inhibit_system_sync);
#endif
}

/**
 * storaged_daemon_util_get_free_mdraid_device:
 *
 ...
 */
gchar *
storaged_daemon_util_get_free_mdraid_device (void)
{
  gchar *ret = NULL;
  gint n;
  gchar buf[PATH_MAX];
...
/**
 * storaged_ata_identify_get_word:
 ...
 */
guint16
storaged_ata_identify_get_word (const guchar *identify_data, guint word_number)
{
