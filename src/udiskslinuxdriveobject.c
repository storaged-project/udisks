                                      ...
                                      gboolean                get_hw)
{
  ...
  for (devices = object->devices; devices; devices = devices->next)
    {
      if (!get_hw ||
          !udisks_linux_device_is_multipath
            (UDISKS_LINUX_DEVICE (devices->data)))
        {
          ret = devices->data;
          break;
        }
  ...
  for (l = objects; l != NULL; l = l->next)
    {
      ...
      skip = (g_strcmp0 (g_udev_device_get_devtype (device->udev_device), "disk") != 0
              || (get_hw && udisks_linux_device_is_multipath (device)));
  ...
  if (vpd == NULL)
    {
      ...
      /* dm-multipath */
      if (udisks_linux_device_is_multipath (device))
        {
          gchar **slaves;
          guint n;
