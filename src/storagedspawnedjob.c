...
/**
 * SECTION:storagedspawnedjob
 * @title: StoragedSpawnedJob
...
typedef struct _StoragedSpawnedJobClass   StoragedSpawnedJobClass;

/**
 * StoragedSpawnedJob:
 *
 * The #StoragedSpawnedJob structure contains only private data and should
...
struct _StoragedSpawnedJobClass
{
  StoragedBaseJobClass parent_class;

  gboolean (*spawned_job_completed) (StoragedSpawnedJob  *job,
                                     GError              *error,
                                     gint                 status,
...
static gboolean storaged_spawned_job_spawned_job_completed_default (StoragedSpawnedJob  *job,
                                                                    GError              *error,
                                                                    gint                 status,
                                                                    GString             *standard_output,
                                                                    GString             *standard_error);

static void storaged_spawned_job_release_resources (StoragedSpawnedJob *job);

G_DEFINE_TYPE_WITH_CODE (StoragedSpawnedJob, storaged_spawned_job, STORAGED_TYPE_BASE_JOB,
                         G_IMPLEMENT_INTERFACE (STORAGED_TYPE_JOB, job_iface_init));

static void
storaged_spawned_job_finalize (GObject *object)
{
  StoragedSpawnedJob *job = STORAGED_SPAWNED_JOB (object);

  storaged_spawned_job_release_resources (job);
  ...
  if (G_OBJECT_CLASS (storaged_spawned_job_parent_class)->finalize != NULL)
    G_OBJECT_CLASS (storaged_spawned_job_parent_class)->finalize (object);
}

static void
storaged_spawned_job_get_property (GObject    *object,
                                   guint       prop_id,
                                   GValue     *value,
                                   GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_COMMAND_LINE:
      g_value_set_string (value, storaged_spawned_job_get_command_line (job));
      break;
...
static void
storaged_spawned_job_set_property (GObject      *object,
                                   guint         prop_id,
                                   const GValue *value,
                                   GParamSpec   *pspec)
{
...
static void
emit_completed_with_error_in_idle (StoragedSpawnedJob *job,
                                   GError             *error)
{
  EmitCompletedData *data;
  GSource *idle_source;

  g_return_if_fail (STORAGED_IS_SPAWNED_JOB (job));
...
static void
storaged_spawned_job_constructed (GObject *object)
{
  StoragedSpawnedJob *job = STORAGED_SPAWNED_JOB (object);
  GError *error;
  gint child_argc;
  ...
  error = NULL;
  if (g_cancellable_set_error_if_cancelled (storaged_base_job_get_cancellable (STORAGED_BASE_JOB (job)), &error))
    {
      emit_completed_with_error_in_idle (job, error);
      g_error_free (error);
      goto out;
  ...
  job->cancellable_handler_id = g_cancellable_connect (storaged_base_job_get_cancellable (STORAGED_BASE_JOB (job)),
...
static void
storaged_spawned_job_init (StoragedSpawnedJob *job)
{
...
static void
storaged_spawned_job_class_init (StoragedSpawnedJobClass *klass)
{
  GObjectClass *gobject_class;

  klass->spawned_job_completed = storaged_spawned_job_spawned_job_completed_default;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize     = storaged_spawned_job_finalize;
  gobject_class->constructed  = storaged_spawned_job_constructed;
  gobject_class->set_property = storaged_spawned_job_set_property;
  gobject_class->get_property = storaged_spawned_job_get_property;

  /**
   * StoragedSpawnedJob:command-line:
   *
  ...
  /**
   * StoragedSpawnedJob:input-string:
   *
  ...
  /**
   * StoragedSpawnedJob:run-as-euid:
   *
  ...
  /**
   * StoragedSpawnedJob::spawned-job-completed:
   * @job: The #StoragedSpawnedJob emitting the signal.
   ...
   *
   * The default implementation simply emits the #StoragedJob::completed
  ...
  signals[SPAWNED_JOB_COMPLETED_SIGNAL] =
    g_signal_new ("spawned-job-completed",
                  STORAGED_TYPE_SPAWNED_JOB,
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (StoragedSpawnedJobClass, spawned_job_completed),
                  g_signal_accumulator_true_handled,
                  NULL,
                  storaged_daemon_marshal_BOOLEAN__BOXED_INT_BOXED_BOXED,
                  G_TYPE_BOOLEAN,
                  4,
                  G_TYPE_ERROR,
                  G_TYPE_INT,
 ...
 *
 * Creates a new #StoragedSpawnedJob instance.
 *
 ...
 *
 * Returns: A new #StoragedSpawnedJob. Free with g_object_unref().
 */
StoragedSpawnedJob *
storaged_spawned_job_new (const gchar    *command_line,
                          ...
                          GCancellable   *cancellable)
{
  ...
  return STORAGED_SPAWNED_JOB (g_object_new (STORAGED_TYPE_SPAWNED_JOB,
...
/**
 * storaged_spawned_job_get_command_line:
 * @job: A #StoragedSpawnedJob.
 *
 ...
 */
const gchar *
storaged_spawned_job_get_command_line (StoragedSpawnedJob *job)
{
  g_return_val_if_fail (STORAGED_IS_SPAWNED_JOB (job), NULL);
...
static gboolean
storaged_spawned_job_spawned_job_completed_default (StoragedSpawnedJob  *job,
                                                    GError              *error,
                                                    gint                 status,
                                                    GString             *standard_output,
                                                    GString             *standard_error)
{
#if 0
  g_debug ("in storaged_spawned_job_spawned_job_completed_default()\n"
           " command_line `%s'\n"
           " error->message=`%s'\n"
  ...
  if (error != NULL)
    {
      ...
      message = g_strdup_printf ("%s (%s, %d)",
                                 ...
                                 error->code);
      storaged_job_emit_completed (STORAGED_JOB (job),
                                 FALSE,
                                 message);
      g_free (message);
    }
...
static void
storaged_spawned_job_release_resources (StoragedSpawnedJob *job)
{
