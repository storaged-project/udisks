...
#define STORAGED_ATA_DEFAULT_COMMAND_TIMEOUT_MSEC (5 * 1000)
...
/**
 * storaged_ata_send_command_sync:
 ...
 */
gboolean
storaged_ata_send_command_sync (gint                       fd,
                                gint                       timeout_msec,
                                ...
                                GError                   **error)
{
  ...
  if (timeout_msec == -1)
    timeout_msec = STORAGED_ATA_DEFAULT_COMMAND_TIMEOUT_MSEC;
  ...
  if (use_ata12)
    {
      ...
      switch (protocol)
        {
        case STORAGED_ATA_COMMAND_PROTOCOL_NONE:
          cdb[1] = 3 << 1;                  /* PROTOCOL: Non-data */
          cdb[2] = 0x20;                    /* OFF_LINE=0, CK_COND=1, T_DIR=0, BYT_BLOK=0, T_LENGTH=0 */
          break;
        case STORAGED_ATA_COMMAND_PROTOCOL_DRIVE_TO_HOST:
          cdb[1] = 4 << 1;                  /* PROTOCOL: PIO Data-In */
          cdb[2] = 0x2e;                    /* OFF_LINE=0, CK_COND=1, T_DIR=1, BYT_BLOK=1, T_LENGTH=2 */
          break;
        case STORAGED_ATA_COMMAND_PROTOCOL_HOST_TO_DRIVE:
  ...
  if (!(sense[0] == 0x72 && desc[0] == 0x9 && desc[1] == 0x0c))
    {
      gchar *s = storaged_daemon_util_hexdump (sense, 32);
      g_set_error (error, G_IO_ERROR, G_IO_ERROR_FAILED,
