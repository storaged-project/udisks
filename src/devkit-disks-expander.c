...
devkit_disks_expander_class_init (DevkitDisksExpanderClass *klass)
{
        ...
        g_object_class_install_property (
                object_class,
                PROP_REVISION,
                g_param_spec_string ("revision", NULL, NULL, NULL, G_PARAM_READABLE));
        g_object_class_install_property (
                object_class,
                PROP_NUM_PORTS,
        ...
        g_object_class_install_property (
                object_class,
                PROP_UPSTREAM_PORTS,
                g_param_spec_boxed ("upstream-ports", NULL, NULL,
...
register_disks_expander (DevkitDisksExpander *expander)
{
        ...
        /* safety first */
        if (dbus_g_connection_lookup_g_object (expander->priv->system_bus_connection,
...
devkit_disks_expander_removed (DevkitDisksExpander *expander)
{
        ...
        dbus_g_connection_unregister_g_object (expander->priv->system_bus_connection,
                                               G_OBJECT (expander));
        g_assert (dbus_g_connection_lookup_g_object (expander->priv->system_bus_connection,
                                                     expander->priv->object_path) == NULL);
}
...
update_info (DevkitDisksExpander *expander)
{
        ...
        if (expander->priv->native_path_for_sysfs_prefix == NULL) {
                expander->priv->native_path_for_sysfs_prefix =
                        sysfs_resolve_link (g_udev_device_get_sysfs_path (expander->priv->d), "device");
                if (expander->priv->native_path_for_sysfs_prefix == NULL) {
                        g_warning ("Unable to resolve 'device' symlink for %s",
                                   g_udev_device_get_sysfs_path (expander->priv->d));
                        goto out;
                }
        }
